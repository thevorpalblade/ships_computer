...
void setup() {
  ...
  sensesp_app = (&builder)
                    ...
                    ->set_wifi_client("launch control", "GodIsLove!")  // Uncomment and set your WiFi credentials
                    //->set_wifi_access_point("My AP SSID", "my_ap_password")
                    ->set_sk_server("172.24.0.16", 3000)
                    ->set_button_pin(-1)
                    ->get_app();

  // GPIO number to use for the analog input
  const uint8_t kAnalogInputPin = 2;
  ...
  const float kAnalogInputScale = 3.3 / 4095.0;
  ...
  // Add LED control via SignalK PUT requests
  const uint8_t kLedPin = 3;
  pinMode(kLedPin, OUTPUT);

  auto led_listener = std::make_shared<BoolSKPutRequestListener>("electrical.switches.test_led.state");
  led_listener->connect_to(std::make_shared<LambdaConsumer<bool>>([kLedPin](bool state) {
    digitalWrite(kLedPin, state ? HIGH : LOW);
    debugD("LED set to: %s", state ? "ON" : "OFF");
  }));
